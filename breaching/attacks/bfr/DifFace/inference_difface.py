...
def main():
    ...
    parser.add_argument(
            "--needsr",
            action='store_true',
            help='Whether to use SR to restore the low quality image',
            )
    ...
    for ii, im_path in enumerate(im_path_list):
        ...
        import cv2 
        save_img_size = cv2.imread(str(im_path)).shape[0]
        # im_lq = util_image.imread(im_path, chn='bgr', dtype='uint8', target_size=(align_img_size, align_img_size))
        ...
        if args.aligned:
            ...
            save_path = restored_face_dir / im_path.name.split('.')[0] / "0.png"
            util_image.imwrite(face_restored, save_path, chn='bgr', dtype_in='uint8', target_size=(save_img_size, save_img_size))
        else:
            ...
            util_image.imwrite(image_restored, save_path, chn='bgr', dtype_in='uint8', target_size=(save_img_size, save_img_size))
