...
from breaching.attacks.bfr.CodeFormer.facelib.detection import init_detection_model
from breaching.attacks.bfr.CodeFormer.facelib.parsing import init_parsing_model
from breaching.attacks.bfr.CodeFormer.facelib.utils.misc import img2tensor, imwrite, is_gray, bgr2gray, adain_npy
...
class FaceRestoreHelper(object):
    ...
    def get_face_landmarks_5(self,
        ...
        with torch.no_grad():
            # reduce the bbox confidence untile detect at least one face            
            bboxes = []
            conf_threshold = 0.8
            while(len(bboxes) == 0):
                bboxes = self.face_detector.detect_faces(input_img, conf_threshold=conf_threshold)
                conf_threshold -= 0.1
    ...
    def align_warp_face(self, save_cropped_path=None, border_mode='constant'):
        ...
        for idx, landmark in enumerate(self.all_landmarks_5):
            ...
            affine_matrix = cv2.estimateAffinePartial2D(landmark, self.face_template, method=cv2.LMEDS)[0] # compare with standard landmarks
