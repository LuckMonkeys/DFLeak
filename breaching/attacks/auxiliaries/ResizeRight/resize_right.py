...
def get_field_of_view(projected_grid, cur_support_sz, in_sz, fw, eps, device):
    ...
    field_of_view = mirror[fw.remainder(field_of_view, mirror.shape[0]).to(mirror.device)] # add .to(mirror.device) make sure same device of indices and indexed tensor
