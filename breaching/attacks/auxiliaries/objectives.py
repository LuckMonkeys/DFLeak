...
class GradientLoss(torch.nn.Module):
    ...
    def forward(self, model, gradient_data, candidate, labels, layer_weights=None):
        ...
        #     #add task loss with transforms
        #     transforms = [torchvision.transforms.RandomHorizontalFlip(),
        #                 # torchvision.transforms.RandomResizedCrop((candidate.shape[2], candidate.shape[3]))]
        #                   torchvision.transforms.RandomRotation(degrees=30),
        #                 #   torchvision.transforms.RandomAffine(degrees=0, translate=(0.1, 0.1)),
        #             ]
        #     weights = [1 for ix in range(len(transforms))]
        #     # aug_task_loss = 0

        #     for aug, weight in zip(transforms, weights):
        #         task_loss += self.loss_fn(model(aug(candidate)), labels) * weight
