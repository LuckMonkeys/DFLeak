...
class _BaseAttacker:
    ...
    def __init__(self, model, loss_fn, cfg_attack, setup=dict(dtype=torch.float, device=torch.device("cpu"))):
        self.cfg = cfg_attack
        self.memory_format = torch.channels_last if cfg_attack.impl.mixed_precision else torch.contiguous_format # for accelerate
    ...
    def prepare_attack(self, server_payload, shared_data):
        ...
        if shared_data[0]["metadata"]["labels"] is None:
            ...
            log.info(f"Provide labels {labels.tolist()}")
    ...
    def _initialize_data(self, data_shape, **kwargs):
        ...
        elif "patch" in init_type:  # Look for init_type=randn-patch-4
            import math
            patch_size = int("".join(filter(str.isdigit, init_type)))
            width, height= math.ceil(data_shape[2] / patch_size), math.ceil(data_shape[3] / patch_size)
            if "randn" in init_type:
                seed = torch.randn([data_shape[0], 3, width, height], **self.setup)
            elif "rand" in init_type:
                seed = (torch.rand([data_shape[0], 3, width, height], **self.setup) * 2) - 1
            else:
                raise AssertionError(f"Wrong init type {init_type}")

            candidate = seed.repeat_interleave(patch_size, dim=2).repeat_interleave(patch_size, dim=3).contiguous().clone()
            # breakpoint() # check init image
        #init from existing images
        
        elif "real-random" == init_type:
            import random 

            # the initialization size of real samples
            if not "cfg_data" in kwargs.keys():
                raise AssertionError("Please provide the cfg_data config for real initialization")
            cfg_data = kwargs["cfg_data"]
            aux_info =tensor2list(kwargs["aux_info"]) # labels for now
            
            split = "train" if cfg_data.examples_from_split=="valid" else "valid"
            ...
            if cfg_data.modality == "vision":
                ...
                #collate_fn used for construct dataloader: i.e. {input=x, labels=y}
                dataset, collate_fn = _build_dataset_vision(cfg_data, split=split, can_download=True)
                ...
                dataset, collate_fn = _build_and_split_dataset_text(
                    cfg_data, split=split, user_idx=0, return_full_dataset=False,
                )
            else:
                raise ValueError(f"Unknown data modality {cfg_data.modality}.")

            dataset.transform = _parse_data_augmentations(cfg_data=cfg_data, split="valid") # only use valid transform for random images

            #load sample from dataset
            initialization_list = []
            for aux in aux_info:
                sample_idx = random.choice(range(len(dataset)))
                initialization_list.append(dataset[sample_idx][0])
                 
            candidate = torch.stack(initialization_list, dim=0).to(**self.setup)
        elif "real-specify" == init_type:
            if not "cfg_data" in kwargs.keys():
                raise AssertionError("Please provide the cfg_data config for real initialization")
            import os, torchvision
            from PIL import Image
            cfg_data = kwargs["cfg_data"]
            user_idx = kwargs["user_idx"] # get the specify image for different user
            img_base_dir = kwargs["img_base_dir"]
            aux_info = tensor2list(kwargs["aux_info"]) 

            
            from breaching.cases.data.datasets_vision import _parse_data_augmentations
            transforms = _parse_data_augmentations(cfg_data=cfg_data, split="valid") # only use the valid transform for spcified images

            imgs_dir = os.path.join(img_base_dir, f"user{user_idx}")
            # load images from dir
            initialization_list = []
            for i in range(len(aux_info)):
                img_path = os.path.join(imgs_dir, f"{i}.png")
                img = Image.open(img_path)
                initialization_list.append(transforms(img))

            # breakpoint() 
            candidate = torch.stack(initialization_list, dim=0).to(**self.setup)
            #do transformation
            # candidate = transforms(tensor_imgs)
            # breakpoint() #check the shape and transform
    ...
    def _init_optimizer(self, candidate, **kwargs):
        step_size = kwargs["step_size"] if "step_size" in kwargs.keys() else self.cfg.optim.step_size
        max_iterations = kwargs["max_iterations"] if "max_iterations" in kwargs.keys() else self.cfg.optim.max_iterations
