...
def save_summary(cfg, metrics, stats, local_time, original_cwd=True, table_name="breach", hydra_output_dir='.'):
    ...
    for step in range(len(stats["train_loss"])):
        ...
        save_to_table(hydra_output_dir, f"{cfg.attack.type}_convergence_results", dryrun=cfg.dryrun, **iteration)
    ...
    location = os.path.join(cfg.original_cwd, "tables") if original_cwd else f"{hydra_output_dir}/tables"
...
hydra_output_dir='.'):
    ...
    os.makedirs(f"{hydra_output_dir}/reconstructions", exist_ok=True)
    ...
    if metadata["modality"] == "text":
        ...
        filepath = os.path.join(
            f"{hydra_output_dir}/reconstructions", f"text_rec_{cfg.case.data.name}_{cfg.case.model}_user{cfg.case.user.user_idx}.txt"
        )
        ...
        filepath = os.path.join(
            f"{hydra_output_dir}/reconstructions", f"img_rec_{cfg.case.data.name}_{cfg.case.model}_user{cfg.case.user.user_idx}.png",
        )
