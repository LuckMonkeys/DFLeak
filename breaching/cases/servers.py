...
class HonestServer:
    ...
    def reconfigure_model(self, model_state, query_id=0, **kwargs):
        ...
        if model_state == "multiple_updates":
            import os
            from glob import glob
            from natsort import os_sorted
            path = kwargs["path"] 
            # print(path)
            if os.path.isdir(path): # the dir of state_dicts
                state_dict_path_list = os_sorted(glob(os.path.join(path, "epoch*.ckpt")))[::-1] # reverse order
                if query_id >= len(state_dict_path_list):
                    raise Exception(f"Query {query_id} out of the length of total state dict {len(state_dict_path_list)}")
                self.model.load_state_dict(torch.load(state_dict_path_list[query_id])["state_dict"])
                print(f"load state dict from {os.path.join(path[0], state_dict_path_list[query_id])}")
        if model_state == "multiple_updates_same":
            # only use the first state dict
            import os
            from glob import glob
            from natsort import os_sorted
            path = kwargs["path"] 
    ...
    def distribute_payload(self, query_id=0, **kwargs):
        """Server payload to send to users. These are only references to simplfiy the simulation."""

        self.reconfigure_model(self.cfg_server.model_state, query_id, **kwargs)
    ...
    def run_protocol(self, user, **kwargs):
        """Helper function to simulate multiple queries given a user object."""
        # Simulate a simple FL protocol
        shared_user_data = []
        payloads = []
        for query_id in self.queries():
            server_payload = self.distribute_payload(query_id, **kwargs)  # A malicious server can return something "fun" here
