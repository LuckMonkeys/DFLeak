...
def _construct_vision_model(cfg_model, cfg_data, pretrained=True, **kwargs):
    ...
    if "ImageNet" or 'Celeba' in cfg_data.name:
        try:
            ...
            model = getattr(torchvision.models, cfg_model.lower())(pretrained=pretrained, num_classes=classes)
            if pretrained:
                if cfg_model.lower() == 'resnet18':
                    # home = Path.home().as_posix()
                    home = '/home/zx'
                    state_dict = torch.load(home + '/.cache/torch/hub/checkpoints/resnet18-f37072fd.pth')
            ...
            #     pass
            # print(cfg_model.lower(), cfg_data.name, classes, model)
        except AttributeError:
            if "facenet-resnet" in cfg_model.lower():
                model = InceptionResnetV1(pretrained=None, classify=True, num_classes=classes)
            elif "facenet-mobilenet" in cfg_model.lower():
                model = Facenet(backbone="mobilenet", num_classes=classes, pretrained=pretrained)
            elif "vggface" in cfg_model.lower():
                from .vggface import VGGFace
                model = VGGFace(num_classes=classes)
